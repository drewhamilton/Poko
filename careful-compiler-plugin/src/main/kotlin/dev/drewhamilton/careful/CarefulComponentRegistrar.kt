...
class CarefulComponentRegistrar : ComponentRegistrar {

    override fun registerProjectComponents(project: MockProject, configuration: CompilerConfiguration) {
        if (configuration[CarefulCommandLineProcessor.KEY_ENABLED] == false)
            return

        // TODO: Use ClassBuilderInterceptorExtension, ExpressionCodegenExtension, or both?

        val messageCollector = configuration.get(CLIConfigurationKeys.MESSAGE_COLLECTOR_KEY, MessageCollector.NONE)
        ExpressionCodegenExtension.registerExtensionAsFirst(project, CarefulCodegenExtension(messageCollector))
    }
